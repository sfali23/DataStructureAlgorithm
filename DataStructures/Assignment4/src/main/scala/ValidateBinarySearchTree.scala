...
/**
  * @author sali
  */
trait ValidateBinarySearchTreeWork {
  ...
  def isBinarySearchTree(root: Option[Node]): Boolean = isBinarySearchTree(root, Long.MinValue, Long.MaxValue)
  ...
  private def runTest(testIndex: Int): Unit = {
    val fileNamePrefix = f"$testIndex%02d"
    System.setIn(ValidateBinarySearchTree.getClass.getResourceAsStream(s"/is-bst/$fileNamePrefix"))
    solve()
  }
...
object ValidateBinarySearchTree extends App with ValidateBinarySearchTreeWork {
  solve()
}
